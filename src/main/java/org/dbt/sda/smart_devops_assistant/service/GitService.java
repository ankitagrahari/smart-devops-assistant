...
public class GitService {
    ...
    public ResponseEntity<String> fetchPRFiles(String prNumber){
        if(Objects.nonNull(prNumber)){
            HttpEntity httpEntity = generateHttpEntity();
            String gitURL = GITHUB_API_URL + "repos/" + GIT_OWNER + "/" + GIT_REPO_NAME + "/" + prNumber + "/files";
            logger.debug("Fetch PR files: gitURL:{}", gitURL);
            return restTemplate.exchange(gitURL, HttpMethod.GET, httpEntity, String.class);
        }
        return ResponseEntity.noContent().build();
    }
